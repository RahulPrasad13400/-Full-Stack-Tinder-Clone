...
const MessageInput = ({ match }) => {
  ...
  return (
    <form onSubmit={handleSendMessage} className="flex relative">
      ...
      <button
        type="submit"
        className="bg-pink-500 text-white p-3 rounded-r-lg hover:bg-pink-600 transition-colors focus:outline-none focus:ring-2 focus:ring-pink-300 flex items-center"
      >
        <Send size={24} />
      </button>
      {showEmojiPicker && (
        <div ref={emojiPickerRef} className="absolute left-4 bottom-20">
          <EmojiPicker
